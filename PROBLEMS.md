...
## 5. Cross-platform passive capture 확장 (4-30 form catch, **SHIPPED 0.9.0**)

### 발견
세 platform 다 session UUID + per-message UUID + transcript 파일을 자기 home 아래 보유. Claude Code만 jsonl_capture로 잡고 있어서 codex/gemini는 save_message tool 호출 의존 → external_uuid NULL + retry 시 중복 row 위험. form 직관 catch:
> "코덱스도 세션 끝낼때 'codex resume <uuid>'로 나오는데 그럼 uuid 잡을 수 있단 소리 아니야?"
> "제미니도 'gemini --resume <uuid>' 여기 보면 uuid가 있는데"

### Inventory (4-30 실측)
| Platform | Transcript 경로 | 포맷 | Per-msg UUID |
|---|---|---|---|
| Claude Code | `~/.claude/projects/<slug>/<sid>.jsonl` | JSONL flat (`type: user` / `assistant` 한 줄씩) | `entry.uuid` ✓ (캡처중) |
| Codex | `~/.codex/sessions/<YYYY>/<MM>/<DD>/rollout-<ts>-<sid>.jsonl` | JSONL nested types (`session_meta`, `event_msg`, `response_item` 등). 사용자 입력은 `response_item` payload `role: user/developer` + `content[].type: input_text`. 어시스턴트는 `event_msg` `payload.type: agent_message` + `payload.message`. | response_item에 turn_id 등 — 별도 식별자 추출 필요 |
| Gemini CLI | `~/.gemini/tmp/<projectHash>/chats/session-<ts>-<short>.json` | 단일 JSON 파일. `messages[]` 배열, 각 entry `{id, timestamp, type: 'user'\|'gemini', content}` | `messages[].id` ✓ |

### 구현 계획 (별도 ship)
- `src/auto_save/codex_capture.ts` — `~/.codex/sessions/` dir watch + nested type parser
- `src/auto_save/gemini_capture.ts` — `~/.gemini/tmp/<hash>/chats/` dir watch + JSON file delta detect (cursor = lastUpdated 또는 messages.length)
- `index.ts`: 각 capture 의 startup/shutdown wiring
- external_uuid 컨벤션: `codex:<sid>:<msg-id>`, `gemini:<sid>:<msg-id>`

### 효과
- 모든 platform external_uuid populated → 중복 row 0건 보장
- save_message tool은 진짜 fallback 위치로 (Cursor 같은 transcript-less platform 한정)
- codex/gemini approval prompt 무관 (passive read니까 caller 호출 자체 안 함) — `manage_knowledge` 같은 명시 변경만 prompt
- form vision (자동 + cross-platform unified) 완성도 ↑

### Why 별도 ship
- 각 platform parser 형태 달라 작업 규모 큼 (~300-500 lines)
- 0.8.3은 Claude Code path 정합화 + brief/null fix로 이미 응집 ship
- 0.9.0 minor bump이 정합 (새 platform 자동 캡처 = 새 기능군)
- skill 알고리즘 회의가 더 high-impact + 우선순위

### 새 세션에서 picking up할 때
- 본 §5 + `feedback_no_settings_json_writes.md` (호스트 config 안 건드리는 원칙) 같이 read
- 새 capture 모듈은 jsonl_capture.ts (multi-file dir watch + per-file cursor 패턴) 그대로 복제 + 각 platform별 parseEntry만 다르게
- shutdown drain race 패턴 (worker 먼저 stop → drain) 동일 적용

### 0.9.0 ship 결과 (2026-04-30)
- `src/auto_save/codex_capture.ts` (~370 lines) — recursive watch (자정 rollover 안전) + session_meta.cwd filter + turn_context.model 추적 + byte-offset external_uuid
- `src/auto_save/gemini_capture.ts` (~280 lines) — SHA256(cwd) + basename(cwd) 두 chats dir 후보 + array-index cursor + JSON parse retry (mid-write)
- `src/index.ts` — 세 platform 병렬 startup arm + shutdown drain (Promise.allSettled)
- server instructions self-documenting: "transcript 캡처 platform은 save_message 호출 X — fallback 한정"
- form action: 자기 GEMINI.md system prompt에서 매 turn save_message convention 줄 정리 (옵션, server instructions가 우선이면 안 건드려도 OK)
