...
- [ ] **Auto User Profile** — `profile.static`(장기) + `profile.dynamic`(현재 맥락) 자동 합성
...
| **Skill Auditor (`auditSkill`)** | Tavily + Exa parallel search → Sonnet 4.6 reconcile → validation_tier classification, integrated into memory_save_skill (default on) |
...
| 항목 | 상태 |
|---|---|
| Auto User Profile (static/dynamic 분리) | 미구현 |
