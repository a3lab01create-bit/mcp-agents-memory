...
import { updateOrCreateSkill, SkillUpdateResult } from "./skills.js";
import { auditSkill } from "./skill_auditor.js";
import type { ValidationTier } from "./skill_auditor.js";
...
export interface CuratedSkillCandidate {
  ...
  reason: string;
  dry_run: boolean;
  skill_result: SkillUpdateResult | null;
  audit?: {
...
interface NormalizedCuratorOptions {
...
async function resolveSubjectId(subjectKey: string | undefined): Promise<number | null> {
...
function avgImportance(members: ClusterMember[]): number {
...
async function getSeedMemories(options: NormalizedCuratorOptions): Promise<SeedMemory[]> {
  const params: unknown[] = [];
  const conditions = buildMemoryScope(options.subjectId, options.projectId, 'm', params);

  const res = await db.query(
    `SELECT id, content, fact_type, importance, created_at
     FROM memories m
...
async function getClusterMembers(
  seed: SeedMemory,
  usedIds: Set<number>,
  options: NormalizedCuratorOptions,
): Promise<ClusterMember[]> {
  const params: unknown[] = [seed.id, Array.from(usedIds), options.similarityThreshold];
  const conditions = buildMemoryScope(options.subjectId, options.projectId, 'm', params);
  ...
  members.push(...res.rows.map((row: any) => ({
    id: row.id,
    content: row.content,
    fact_type: row.fact_type,
    importance: Number(row.importance),
...
export async function findClusters(options: NormalizedCuratorOptions): Promise<{ clusters: ClusterCandidate[]; scanned: number }> {
...
export async function isClusterAlreadyCovered(cluster: ClusterCandidate): Promise<boolean> {
  const res = await db.query(
    `SELECT source_memory_ids
     FROM skill_changelog
     WHERE source_memory_ids && $1::int[]`,
    [cluster.member_ids]
  );
...
function buildClusterPrompt(cluster: ClusterCandidate): string {
...
function parseCuratorResponse(raw: string): CuratorLLMResponse {
...
export async function runCurator(options: CuratorOptions = {}): Promise<CuratorResult> {
  ...
  const normalized: NormalizedCuratorOptions = {
    subjectId,
    projectId,
