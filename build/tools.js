...
export function registerTools(server) {
    ...
    // ═══════════════════════════════════════════════════════════
    server.registerTool("memory_startup", {
        ...
        try {
            ...
            const staticProfileRes = await db.query(`SELECT content, fact_type, tags
           FROM memories
           WHERE subject_id = $1
           ...
           LIMIT 8`, [userId]);
            if (staticProfileRes.rows.length > 0) {
                ...
                sections.push("───────────────");
                staticProfileRes.rows.forEach((r) => {
                    sections.push(`• [${r.fact_type}] ${r.content}`);
                    if (r.tags?.length > 0)
                        sections.push(`  tags: ${r.tags.join(', ')}`);
                });
                sections.push("");
            }
            const dynamicProfileRes = await db.query(`SELECT content, fact_type, tags
           FROM memories
           WHERE subject_id = $1
             AND fact_type IN ('profile', 'preference')
             AND is_active = TRUE
           ...
           LIMIT 6`, [userId]);
            if (dynamicProfileRes.rows.length > 0) {
                ...
                sections.push("────────────────");
                dynamicProfileRes.rows.forEach((r) => {
                    sections.push(`• [${r.fact_type}] ${r.content}`);
                    if (r.tags?.length > 0)
                        sections.push(`  tags: ${r.tags.join(', ')}`);
                });
        ...
        try {
            const projRes = await db.query(`SELECT s.subject_key, s.display_name,
        ...
        try {
            const decisionRes = await db.query(`SELECT content, fact_type, tags, created_at
           FROM memories
           ...
           LIMIT 5`);
            if (decisionRes.rows.length > 0) {
                ...
                sections.push("───────────────────────────");
                decisionRes.rows.forEach((r) => {
                    const date = new Date(r.created_at).toLocaleDateString('ko-KR');
                    sections.push(`• [${r.fact_type}] ${r.content}`);
                    if (r.tags?.length > 0)
                        sections.push(`  tags: ${r.tags.join(', ')} | ${date}`);
                });
                sections.push("");
            }
        }
        ...
        try {
            ...
            if (skills.length > 0) {
                ...
                sections.push("────────────────");
                skills.forEach((r) => {
                    const tierBadge = r.validation_tier === 'validated_external' ? '🔬' :
        ...
        // 4. Skills
        try {
            const skillRes = await db.query(`SELECT content, tags
           FROM memories
    ...
    // ═══════════════════════════════════════════════════════════
    server.registerTool("memory_save_skill", {
        ...
        if (result.parent_skill_id)
            lines.push(`Parent Skill ID: ${result.parent_skill_id}`);
    ...
    // ═══════════════════════════════════════════════════════════
    server.registerTool("memory_curator_run", {
    ...
    server.registerTool("memory_search", {
        ...
        let baseContext = "";
        try {
            const profileRes = await db.query(`SELECT content, fact_type FROM memories
        ...
        // ── Search Logic ──
        const params = [embeddingSql, args.limit, `%${args.query}%`];
        const conditions = [
            "f.is_active = TRUE",
            `(
        ...
        if (args.subject_key) {
            ...
            if (args.expand_via_graph) {
                const neighbors = await db.query(`SELECT $1::int AS id
             UNION
             SELECT to_subject_id FROM subject_relationships WHERE from_subject_id = $1
             UNION
             SELECT from_subject_id FROM subject_relationships WHERE to_subject_id = $1`, [subId]);
                const ids = neighbors.rows.map((r) => r.id);
                params.push(ids);
        ...
        const results = await db.query(`SELECT f.id, f.content, f.fact_type, f.confidence, f.importance, f.tags, f.created_at,
        ...
        await db.query(`UPDATE memories SET access_count = access_count + 1, last_accessed_at = NOW() WHERE id = ANY($1::int[])`, [ids]);
        ...
        results.rows.forEach((r) => {
            ...
            if (r.subject_name)
                formatted += `Subject: ${r.subject_name}`;
            if (r.project_name)
                formatted += ` | Project: ${r.project_name}`;
            if (r.subject_name || r.project_name)
                formatted += `\n`;
            if (r.tags?.length > 0)
                formatted += `Tags: ${r.tags.join(", ")}\n`;
    ...
    server.registerTool("memory_status", {
        ...
        try {
            ...
            const superseded = await db.query(`SELECT COUNT(*) as count FROM memories
           WHERE is_active = FALSE AND superseded_by IS NOT NULL`);
            ...
            lines.push("⚠️ Could not fetch totals\n");
        }
        // Facts by type
        try {
            const byType = await db.query(`SELECT fact_type, COUNT(*) as count
        ...
        try {
            ...
            const byRel = await db.query(`SELECT relationship_type, COUNT(*)::int AS count
        ...
        try {
            const forgottenTotal = await db.query(`SELECT COUNT(*)::int AS count FROM memories
           WHERE is_active = FALSE AND metadata ? 'forgotten_at'`);
            const byTypeForgotten = await db.query(`SELECT fact_type, COUNT(*)::int AS count FROM memories
           WHERE is_active = FALSE AND metadata ? 'forgotten_at'
           GROUP BY fact_type ORDER BY count DESC`);
            ...
            if (total > 0) {
                ...
                byTypeForgotten.rows.forEach((r) => {
                    lines.push(`  ${r.fact_type}: ${r.count}`);
                });
                lines.push("");
            }
    ...
    // ═══════════════════════════════════════════════════════════
    server.registerTool("connector_sync", {
        description: "Sync external content into memory via a Connector. v1 supports Notion only. Provide either a `page` id (top-level blocks of one page) or a `database` id (iterates child pages). Hash-based dedup via memory_sources table — re-syncing unchanged content is a no-op. Requires NOTION_API_KEY in your config.",
        inputSchema: {
    ...
    // ---------------------------------------------------------
    server.prompt("briefing", "Get a full briefing of the user profile and project state.", {}, (args) => ({
        messages: [{
                role: "user",
                content: {
                    type: "text",
    ...
    server.prompt("recall", "Recall information from long-term memory.", { query: z.string().describe("Search query") }, (args) => ({
        messages: [{
                role: "user",
                content: {
                    type: "text",
    ...
    server.prompt("save", "Save a new fact or decision to memory.", { text: z.string().describe("Information to save") }, (args) => ({
        messages: [{
                role: "user",
                content: {
                    type: "text",
