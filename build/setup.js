        ...
        const { url } = await inquirer.prompt([
            {
                ...
                validate: (v) => /^postgres(ql)?:\/\//.test(v.trim()) || "Must start with postgres:// or postgresql://",
            },
        ]);
...
async function promptForKeys(needs, prompts) {
    const fields = {
    ...
    const questions = prompts
        ...
        .map((k) => ({ type: "password", name: k, mask: "*", message: fields[k].msg }));
    if (questions.length === 0)
        return needs;
    const answers = await inquirer.prompt(questions);
    return { ...needs, ...answers };
}
async function promptCustomRoles() {
...
async function promptModelsAndKeys() {
    console.log("\n🧠 LLM roles");
    ...
    const presetNeeds = preset.id === "custom"
        ? Array.from(new Set(Object.values(roleEnv).map((r) => r.provider)))
        : preset.needs;
    ...
    const { enableGrounding } = await inquirer.prompt([
        {
            ...
            message: "Enable advanced grounding (Tavily + Exa + Claude Sonnet for skill/memory auditors)? Adds ~$15-25/year for typical use, $0 if you skip.",
            default: false,
        },
    ]);
    // Gather all keys we need based on preset + grounding choice (+ Bucket A: OpenAI for embedding).
    const requiredProviders = new Set(["openai", ...presetNeeds]);
    const groundingKeys = enableGrounding ? ["anthropic", "tavily", "exa"] : [];
    ...
    if (askNotion)
        keys = await promptForKeys(keys, ["notion"]);
    // ── Build .env block ──
    const lines = [];
    if (keys.openai)
    ...
    if (keys.google)
        lines.push(`GOOGLE_GENERATIVE_AI_API_KEY=${keys.google}`);
    if (keys.xai)
        lines.push(`GROK_API_KEY=${keys.xai}`);
    if (keys.tavily)
        lines.push(`TAVILY_API_KEY=${keys.tavily}`);
    if (keys.exa)
        lines.push(`EXA_API_KEY=${keys.exa}`);
    if (keys.notion)
        lines.push(`NOTION_API_KEY=${keys.notion}`);
    lines.push("");
...
const invokedDirectly = process.argv[1] && process.argv[1].endsWith("/setup.js");
if (invokedDirectly) {
    runSetupWizard()
        .then(() => process.exit(0))
