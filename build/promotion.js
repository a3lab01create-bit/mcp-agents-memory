...
export function maybeStartPromotionLoop() {
...
async function tickPromotionLoop() {
    if (running) {
    ...
    try {
        ...
        console.error(`🌀 [Promotion] tick done — scanned=${result.scanned_memories}, clusters=${result.clusters_found}, skipped=${result.clusters_skipped}, saved=${result.skills_saved} (in ${durationSec}s)`);
    }
    catch (err) {
...
export function stopPromotionLoop() {
    if (warmupTimer) {
