...
import { fetchPage } from "./notion.js";
const PROVIDER_SUBJECT_KEY = {
    notion: "system_connector_notion",
    github: "system_connector_github",
    drive: "system_connector_drive",
};
function normalizeForHash(plaintext) {
...
async function syncOnePage(provider, resource, result) {
    result.pages_seen++;
    const newHash = hash(resource.plaintext);
    const existing = await db.query(`SELECT id, content_hash FROM memory_sources WHERE provider = $1 AND external_id = $2`, [provider, resource.external_id]);
...
async function upsertSource(provider, resource, contentHash, factsAdded) {
    await db.query(`INSERT INTO memory_sources
...
export async function runConnectorSync(req) {
    const result = {
        pages_seen: 0,
        pages_synced: 0,
    ...
    throw new Error(`Unsupported resource_type '${req.resource_type}' for Notion in v1 — only 'page' is implemented.`);
}
