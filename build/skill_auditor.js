...
function truncateForSearch(candidate) {
...
function normalizeApplicability(raw) {
    ...
    if (Array.isArray(obj.models)) {
        const models = obj.models.filter((v) => typeof v === 'string' && v.length > 0);
        if (models.length > 0)
            result.models = Array.from(new Set(models));
    }
    if (Array.isArray(obj.platforms)) {
        const platforms = obj.platforms.filter((v) => typeof v === 'string' && v.length > 0);
        if (platforms.length > 0)
            result.platforms = Array.from(new Set(platforms));
    }
    return result;
}
function normalizeAuditorResponse(raw) {
    ...
    const validationTier = VALIDATION_TIERS.includes(parsed.validation_tier)
        ? parsed.validation_tier
        : 'contested';
...
export async function auditSkill(candidate) {
...
SOURCES:
${searchResults.map((source, index) => `[${index}] (engine=${source.engine}, authority=${source.authority}, weight=${source.weight}) ${source.title}: ${source.snippet}`).join("\n")}`;
    try {
