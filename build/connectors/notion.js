...
function getClient() {
    ...
    if (!auth) {
        throw new Error("NOTION_API_KEY missing. Add it to your config (~/.config/mcp-agents-memory/.env) " +
            "or set it in the shell before invoking the connector.");
    }
    cached = new Client({ auth });
    return cached;
}
function richTextToPlaintext(rt) {
    if (!rt)
        return "";
...
function extractTitleFromPage(page) {
    ...
    for (const prop of Object.values(page.properties ?? {})) {
        if (prop.type === "title")
            return richTextToPlaintext(prop.title) || null;
    }
    return null;
}
/**
 ...
 */
async function collectLines(client, blockId, depth, lines) {
    if (depth > 5)
        return; // safety cap on deeply nested toggles
    ...
    for (const b of blocks) {
        ...
        if (line)
            lines.push(line);
        if (b.has_children && RECURSE_INTO.has(b.type)) {
            await collectLines(client, b.id, depth + 1, lines);
        }
    }
}
