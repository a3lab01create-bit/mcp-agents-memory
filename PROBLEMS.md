...
- **4-30 2차 ship (B1)**: Claude Code **Stop hook** 실시간 캡처 도입. 매 assistant turn 종료 시 fire → JSONL delta 즉시 INSERT.
  - 새 CLI: `mcp-agents-memory install-hooks` / `uninstall-hooks` / `capture-session`
  - 새 파일: `src/auto_save/install_hooks.ts` (settings.json patcher) + `src/auto_save/capture_session.ts` (Stop hook 진입점)
  - cursor: `~/.cache/mcp-agents-memory/cursors/<session_id>.cursor` (byte offset persist)
  - dedup: 기존 `external_uuid claude-code:<sid>:<entry-uuid>` 그대로 → server-shutdown path와 동시 활성 안전
  - install-hooks가 dead `memory_add` SessionEnd hook 검출 시 경고만 (자동 제거 X — 사용자 수동 처리)
  - 검증: fake transcript fixture로 inserted/dedup/cursor advance/no-delta-skip 4가지 시나리오 모두 통과
- form 직접 1회 실행 필요: `node build/index.js install-hooks` (Auto Mode가 settings.json modify 막아서 form 명시 실행)

### 조심
- JSONL 캡처는 form 머신에 누적된 옛 JSONL 파일들 backfill 안 하도록 cursor (서버 시작 시점 byte) 명시.
- save_message tool은 instructions에 호출 convention 명시했을 때만 신뢰. agent가 잊으면 누락. Stop hook 캡처가 Claude Code 한정 보조.
- Stop hook은 Claude Code 한정. **Gemini CLI / Codex 등은 `save_message` tool + instructions에 의존** (PROBLEMS.md §4 (C) 합의 그대로, B1 cover 범위 밖).
- Stop hook timeout 30s 박힘 — JSONL 작거나 캡처 빠르면 충분. SSH tunnel cold start 포함 시 1~2s 측정값.
