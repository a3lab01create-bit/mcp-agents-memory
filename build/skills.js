...
async function findMostSimilarSkill(embeddingSql) {
    if (!embeddingSql)
        return null;
    const res = await db.query(`SELECT id, title, parent_skill_id,
            1 - (embedding <=> $1::vector) AS similarity
     FROM skills
     WHERE status = 'active'
       AND embedding IS NOT NULL
     ORDER BY embedding <=> $1::vector
     LIMIT 1`, [embeddingSql]);
...
export async function getInjectableSkills(ctx) {
    const limit = ctx.limit ?? 5;
    const res = await db.query(`SELECT id, title, content, validation_tier, use_count, last_used_at
     FROM skills
     WHERE status = 'active'
       AND (
         applicable_to = '{}'::jsonb
     ...
     LIMIT $3`, [ctx.author_model ?? null, ctx.platform ?? null, limit]);
...
export async function recordSkillExposure(skillIds) {
    if (skillIds.length === 0)
        return;
    await db.query(`UPDATE skills
     SET last_used_at = NOW()
     WHERE id = ANY($1::int[])`, [skillIds]);
}
export async function updateOrCreateSkill(candidate, audit) {
    ...
    try {
        ...
        if (match && similarity >= ACCUMULATE_THRESHOLD) {
            await client.query(`UPDATE skills
         SET use_count = use_count + 1,
             last_used_at = NOW()
         WHERE id = $1`, [match.id]);
            await client.query(`INSERT INTO skill_changelog (
        ...
        if (match && similarity >= BRANCH_THRESHOLD) {
         ...
         SET status = 'inactive'
         WHERE id = $1`, [match.id]);
            const insertRes = await client.query(`INSERT INTO skills (
