...
// Pipeline Stages
// ─────────────────────────────────────────────────────────────
export function computeAuditScore(fact) {
...
function normalizeProfileAxisTags(fact) {
...
export async function triageTranscript(text) {
    try {
...
export async function auditFacts(facts) {
...
export async function resolvePlatform(name) {
    ...
    if (PLATFORM_CACHE.has(key))
        return PLATFORM_CACHE.get(key);
    // Try exact match first
    const sel = await db.query("SELECT id, name, trust_weight FROM platforms WHERE LOWER(name) = $1 LIMIT 1", [key]);
    if (sel.rows.length > 0) {
        const r = { id: sel.rows[0].id, name: sel.rows[0].name, trust_weight: parseFloat(sel.rows[0].trust_weight) };
        PLATFORM_CACHE.set(key, r);
        return r;
    }
    // Auto-register with default trust_weight=1.00
    const ins = await db.query("INSERT INTO platforms (name, trust_weight) VALUES ($1, 1.00) ON CONFLICT (name) DO UPDATE SET name = EXCLUDED.name RETURNING id, name, trust_weight", [name]);
...
export async function resolveModel(name) {
    ...
    try {
        const res = await db.query(`SELECT id, model_name, trust_weight
       FROM models
 ...
 */
export function computeEffectiveConfidence(confidence, trustWeight) {
...
async function resolveContradiction(newFact, subjectId, existingEmbedding) {
    try {
        ...
        if (!embedding)
            return { supersedes_id: null, reason: "No embedding" };
        const similar = await db.query(`SELECT id, content FROM memories
       WHERE subject_id = $1 AND is_active = TRUE AND fact_type = $2 
       ORDER BY embedding <=> $3::vector LIMIT 5`, [subjectId, newFact.fact_type, vectorToSql(embedding)]);
        if (similar.rows.length === 0)
            return { supersedes_id: null, reason: "No similar facts" };
...
// ─────────────────────────────────────────────────────────────
const AUDITABLE_FACT_TYPES = new Set(['learning']);
function memoryAuditEnabled() {
    const flag = (process.env.MEMORY_AUDIT_ENABLED || '').toLowerCase();
    return flag === '1' || flag === 'true' || flag === 'yes';
}
function shouldAuditFact(fact) {
    if (!memoryAuditEnabled())
        return false;
    if (!AUDITABLE_FACT_TYPES.has(fact.fact_type))
        return false;
...
async function persistAuditResult(factId, audited, originalContent) {
    const status = tierToStatus(audited.validation_tier);
    if (status === null)
        return; // skip persistence for unvalidated
    const client = await db.getClient();
    try {
        await client.query('BEGIN');
        await client.query(`INSERT INTO fact_validations (fact_id, status, confidence_score, research_report, metadata)
       VALUES ($1, $2, $3, $4, $5)`, [
            factId,
        ...
        await client.query(`UPDATE memories SET validation_status = $1, last_validated_at = NOW() WHERE id = $2`, [status, factId]);
 ...
 */
async function upsertSubjectEdge(edge) {
    ...
    try {
        ...
        if (fromId === toId)
            return false; // self-loop blocked by CHECK anyway
        const res = await db.query(`INSERT INTO subject_relationships (from_subject_id, to_subject_id, relationship_type)
       VALUES ($1, $2, $3)
       ON CONFLICT (from_subject_id, to_subject_id, relationship_type) DO NOTHING
       RETURNING id`, [fromId, toId, edge.type]);
...
export async function processBatch(text, subjectId, projectId, rawSource, provenance = {}) {
    const result = { extracted: 0, saved: 0, contradictions_resolved: 0, edges_saved: 0, audited: 0, sources: [], facts: [], errors: [] };
    try {
        ...
        for (const fact of audited) {
            ...
            const insertRes = await db.query(`INSERT INTO memories (
...
export function scheduleValidation(factContent, factId) {
    validationQueue.enqueue(() => validateFact(factContent, factId)
        .then(() => { })
        .catch(err => console.error(`❌ [ValidationQueue] validateFact failed for memory #${factId}:`, err)));
}
const VALIDATION_LIMIT = parseInt(process.env.VALIDATE_CONCURRENCY || process.env.VALIDATION_CONCURRENCY || '2');
