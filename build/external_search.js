...
export function getAuthority(url) {
    try {
        ...
        if (hostname.endsWith('.org'))
            return 'medium';
        if (/^(en|ko|ja|de|fr)\.wikipedia\.org$/.test(hostname))
            return 'medium';
        if (hostname === 'developer.mozilla.org' || hostname.endsWith('.mozilla.org'))
            return 'medium';
        return 'low';
    }
    catch {
...
export function authorityToWeight(authority) {
...
function dedupeSources(sources) {
    ...
    for (const source of sources) {
        ...
        if (source.weight > existing.weight ||
...
export async function searchExternal(query, opts) {
