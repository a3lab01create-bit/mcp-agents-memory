 ...
 */
export async function validateFact(factContent, factId) {
...
async function saveValidationResult(factId, report) {
    const client = await db.getClient();
    try {
        await client.query('BEGIN');
        // Insert into fact_validations
        const insertRes = await client.query(`INSERT INTO fact_validations (fact_id, status, confidence_score, research_report, metadata)
       VALUES ($1, $2, $3, $4, $5)
       RETURNING id`, [factId, report.status, report.confidence_score, report.reasoning, JSON.stringify({ sources: report.sources })]);
        // Update memories table status
        await client.query(`UPDATE memories SET validation_status = $1, last_validated_at = NOW() WHERE id = $2`, [report.status, factId]);
